...
// Produtos
ipcMain.on('search-product', async (event, {termo, tipo}) => {
    try {
        let query = {};
        if(tipo === 'nome') {
            query.nomeProduto = new RegExp(termo, 'i');
        } else {
            query.codProduto = termo;
        }
        
        const dadosProduto = await produtoModel.find(query);
...
ipcMain.on('update-supplier', async (event, fornecedor) => {
    try {
        await fornecedorModel.findByIdAndUpdate(
            fornecedor.idForn,
            {
                nomeFornecedor: fornecedor.nomeForn,
                foneFornecedor: fornecedor.foneForn,
...
ipcMain.on('delete-supplier', async (event, id) => {
    try {
        await fornecedorModel.findByIdAndDelete(id);
        event.reply('supplier-deleted');
    } catch (error) {
        console.error(error);
    }
});
